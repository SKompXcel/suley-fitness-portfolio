...
function makeRequest(token: string | null = TOKEN): NextRequest {
  ...
  return new NextRequest('http://localhost/api/admin/ops/queue', {
    method: 'GET',
    headers,
  })
}

describe('GET /api/admin/ops/queue', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  ...
  it('returns up to 5 pending intents, oldest first', async () => {
    const rows = [
      {
        id: 'a',
        kind: 'kb_site_sync',
        requestedAt: new Date('2026-09-05T12:00:00Z'),
      },
    ]
